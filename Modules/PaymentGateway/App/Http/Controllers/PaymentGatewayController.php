...
use Modules\PaymentGateway\App\Http\Requests\PayURequest;


/**
 * PaymentGatewayController
 *
 * Admin configuration for payment gateway credentials and activation.
 *
 * @package Modules\PaymentGateway\App\Http\Controllers
 */
class PaymentGatewayController extends Controller
{
    ...
    public function update_payu(PayURequest $request)
    {
        // Salvăm valorile PayU în baza de date
        PaymentGateway::where('key', 'payu_currency_id')->update(['value' => $request->currency_id]);
        PaymentGateway::where('key', 'payu_merchant_pos_id')->update(['value' => $request->merchant_pos_id]);
        PaymentGateway::where('key', 'payu_secret_key')->update(['value' => $request->secret_key]);
        PaymentGateway::where('key', 'payu_client_id')->update(['value' => $request->client_id]);
        PaymentGateway::where('key', 'payu_client_secret')->update(['value' => $request->client_secret]);
        PaymentGateway::where('key', 'payu_sandbox')->update(['value' => $request->sandbox ? 1 : 0]);
        PaymentGateway::where('key', 'payu_status')->update(['value' => $request->status ? 1 : 0]);

        // Salvăm imaginea PayU (logo-ul)
        $exist_image = PaymentGateway::where('key', 'payu_image')->first();

        if ($request->image) {
            $old_image = $exist_image->value ?? null;
            $new_image = $request->image;
            $ext = $new_image->getClientOriginalExtension();
